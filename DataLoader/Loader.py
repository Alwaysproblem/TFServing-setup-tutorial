...
SPARSES = ('int32', 'int64')
VARLENS = ('stringv', 'int32List', 'int64List', 'float32List', 'float64List')
DENSES = ('float32', 'float64')
...
class TFRecordLoader(FileTypeLoader):
    def __init__(self, file_paths: list, col_type: dict, batch_size: int, label_name=None,
        ...
        self.sparses = [feat for feat in col_type
                        if col_type[feat] in SPARSES]
        self.varlens = [feat for feat in col_type
                        if col_type[feat] in VARLENS]
        self.numerics = [feat for feat in col_type
                        if col_type[feat] in DENSES]
...
class CSVLoader(FileTypeLoader):
    def __init__(self,
        ...
        Args:
            ...
            varlen_ragged: An optional bool. Defualts to True. It decides if the output is ragged tensor, if it set to be false, varlen_filld_value, varlen_max_len will be used.
            varlen_filld_value: An optional string. when varlen_ragged is False, the value is used to filling the varlen tensor, default to b"" or 0.
            varlen_max_len: An optional int. when varlen_ragged is False, the length after filling varlen tensor. default to the max length of a batch varlen tensor.
