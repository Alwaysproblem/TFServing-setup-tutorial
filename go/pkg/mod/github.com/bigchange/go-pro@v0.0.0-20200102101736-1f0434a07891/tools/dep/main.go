...
func main() {
	...
	flag.StringVar(&gopath, "path", "./", "gopath dir")
	flag.Parse()
	if gopath == "./" {
		fmt.Printf("gopath must set!!")
		return
	}
...
func haveTag(dep Dep) bool {
	cmd := exec.Command("git", "-C", gopath+dep.dir, "show-ref", "--hash", "--tags", dep.version)
	out, err := cmd.Output()
	if err != nil {
		return false
	}
...
func haveHash(dep Dep) bool {
	...
	cmd := exec.Command("git", "-C", gopath+dep.dir, "rev-parse", "--verify",
...
func preflightDir(dep Dep) {
	...
	dirStat, err := os.Stat(gopath + dep.dir)
	...
	if !dirStat.IsDir() {
		...
		_, err := os.Stat(filepath.Join(gopath+dep.dir, ".git"))
		if os.IsNotExist(err) {
			...
			dirFiles, err := ioutil.ReadDir(gopath + dep.dir)
			if err != nil {
				fatalf("Unable to determine contents of directory: %s :%s", dep.dir, err)
			}
			if len(dirFiles) == 0 {
				if err := os.Remove(gopath + dep.dir); err != nil {
...
func fetchOrClone(dep Dep) error {
	cmd := exec.Command("git", "-C", gopath+dep.dir,
		"remote", "set-url", "origin", dep.repo)
	err := cmd.Run()
	if err == nil {
		fmt.Printf("Fetching %v (%v)\n", dep.repo, dep.version)
		cmd = exec.Command("git", "-C", gopath+dep.dir, "fetch")
	} else {
		fmt.Printf("Cloning %v\n", dep.repo)
		cmd = exec.Command("git", "clone", dep.repo, gopath+dep.dir)
	}
...
func reset(dep Dep) error {
	cmd := exec.Command("git", "-C", gopath+dep.dir, "reset", "--hard", dep.version)
	err := cmd.Run()
	if err != nil {
